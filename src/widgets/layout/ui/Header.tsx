...
const BackButton: React.FC<{ onClick: () => void }> = ({ onClick }) => (
  <button onClick={onClick} className="text-neutral-600 hover:text-light-text dark:text-dark-text-secondary dark:hover:text-dark-text-primary" aria-label="Indietro">
...
const Header: React.FC = () => {
  ...
  const getTitle = () => {
    ...
    if (path.startsWith('/catalog/')) return 'Dettaglio';
    if (path === '/notifications') return 'Aggiornamenti';
    if (path.startsWith('/notifications/')) return 'Dettaglio';
    if (path === '/settings') return 'Impostazioni';
    if (path === '/settings/legal') return 'Legali';
    if (path === '/settings/report') return 'Segnala';
    if (path === '/settings/about') return 'Info';
        ...
        <div className="relative z-10 flex items-center flex-1 justify-end h-full">
          {isCatalog && (
            <div className="flex-1 flex items-center justify-center gap-1.5 overflow-hidden h-full">
              <span className="text-[10px] text-neutral-500 font-medium whitespace-nowrap">Agg: {formatUpdateDate(lastUpdateDate)}</span>
              <button 
